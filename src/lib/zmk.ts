...
const getZmkPhysicalLayoutAssignment = (studio: boolean) => studio ? `
&default_layout {
...
`);
  ...
  zip.folder('zephyr')?.file('module.yml', `name: zmk-keyboard-${kbName}
build:
  settings:
    board_root: .
`);
...
2. Verify the generated \`build.yaml\` entries:
   \`\`\`yaml
   include:
     - board: ${boardName}
       shield: ${leftShield}
     - board: ${boardName}
       shield: ${rightShield}
   \`\`\`
3. Push the repository to GitHub. The included workflow builds both halves automatically.
4. Download the compiled firmware binaries from the GitHub Actions tab.
...
const generateSplitCustomBoardFiles = (
  ...
  const dtsInclude = `${getCustomBoardDtsInclude(zmkTarget)}${zmkTarget === 'nrf52840'
...
`;
  ...
  const sharedBoardFolder = zip.folder('boards')?.folder(arch)?.folder(kbName);
  if (!sharedBoardFolder) return false;
  const sharedDtsi = `${studio ? `#include "${kbName}-layouts.dtsi"\n` : ''}#include <dt-bindings/zmk/matrix_transform.h>
...
${getZmkPhysicalLayoutAssignment(studio)}`;
  sharedBoardFolder.file(`${kbName}.dtsi`, sharedDtsi);
  if (studio) {
    sharedBoardFolder.file(`${kbName}-layouts.dtsi`, generateZmkPhysicalLayoutDtsi(sortedKeys));
  }
...
`;
    boardFolder.file(`Kconfig.${boardName}`, kconfigBoard);
...
${zmkTarget === 'nrf52840' && side === 'left' ? `config ZMK_USB
    default y
` : ''}
...
`;
    ...
    const hasSideEncoder = (settings.encoders || []).some((encoder, index) => (
      hasCompleteEncoderPins(encoder) && getEncoderSide(settings, keys, index) === side
    ));
...
${studio && side === 'left' ? '  - studio\n' : ''}`;
  ...
  if (configFolder) {
    ...
    configFolder.file(`${leftBoard}.keymap`, `#include "${keymap.filename}"\n`);
...
- \`boards/arm/${kbName}/\`: Contains the shared matrix transform and physical layout definition.
...
## Split Matrix
The shared \`${kbName}.dtsi\` file defines the full matrix transform. Both custom board DTS files include it, and the right board applies \`col-offset = <${useDirectPins ? leftDirectCols : leftPins.cols.length}>\` so local right-side matrix events map into the right side of the shared transform.
...
config BOARD_${kbName.toUpperCase()}
    ...
    imply RETENTION_BOOT_MODE
`;
    boardFolder.file(`Kconfig.${kbName}`, kconfigBoard);
...
${buildExample}
   \`\`\`
3. Push the repository to GitHub. The included workflow builds the firmware automatically.
4. Download the compiled firmware binary from the GitHub Actions tab.
