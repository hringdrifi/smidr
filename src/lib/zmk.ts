...
`;
  ...
  if (configFolder) {
    const keymap = generateKeymapDts(settings, sortedKeys, kbName);
    configFolder.file(`${leftBoard}.keymap`, keymap.content);
    configFolder.file(`${rightBoard}.keymap`, keymap.content);
  }
  zip.file('build.yaml', generateZmkBuildYaml([
    { board: leftBoard },
    { board: rightBoard },
...
## Directory Structure
- \`config/${leftBoard}.keymap\` and \`config/${rightBoard}.keymap\`: Contain the keymap layers defined in Smidr for each board half.
