...
- **保存リビジョン**: 確定した設定・キーの変更に一意な `historyId` を付与し、Undo/Redoでは内容とともに復元する。発行カウンターは履歴外で単調増加させ、分岐後も番号を再利用しない。保存・読込時のリビジョンを履歴外に保持し、現在値との比較で未保存を判定する。復元ドラフトは正式保存まで未保存扱いとする。プレビュー・UIのみの更新および同値更新では番号を発行しない。
...
- **ドキュメント導線:** アプリのヘッダーには公開ドキュメントを新しいタブで開くリンクを常設し、プロジェクトを開いていない状態やデスクトップ版からも参照できるようにする。
...
- **対応MCU**: RMK 0.9 TOMLは、既存のMCU／開発ボード選択肢のうちRP2040、nRF52840／nRF52833／nRF52832（HY0020）、USB対応STM32を対象とする。MCU名は選択した開発ボードから解決し、別のMCU選択値を流用しない。AVR・Kinetis・GD32・WB32などは非対応として表示し、出力を拒否する。nRF52833のGPIOはP0.00–P0.31／P1.00–P1.09とする。RP2040／nRF52840／nRF52833では単体・分割のビルド用ファイルを生成し、その他の対応MCUはTOML設定のみを生成する。
- **Rust API**: RP2040／nRF52840／nRF52833の単体・分割に対応。`src/keymap.rs` のKeyAction配列と、GPIO・マトリクス・ストレージ・USB／BLE・PMW3610・PointingProcessor・watchdog・`run_all!` を含むエントリーポイントを生成する。分割はcentral.rs／peripheral.rsに分ける。変換できないRaw式はエラーとする。同梱TOMLは等価な設定とコンパイル時容量のために使用し、Rustの実行時キー配列やGPIOを上書きしない。
- **ビルド用ファイル**: 両形式ともRP2040／nRF52840／nRF52833ではbuild.rs、memory.x、.cargo/config.toml、rust-toolchain.tomlとエントリーポイントを含める。TOML形式はRMKの設定マクロを使用する。他MCUはTOML設定のみでボード固有コードを別途用意する。RP2040はFlash 2 MiBを仮定する。nRF52840は0xE0000から、nRF52833は0x78000から8セクタをストレージ用に確保する。nRF52840はAdafruit UF2使用時に0x1000、それ以外は0x0からコードを配置する。実機のブートローダーとの一致はREADMEで確認を促す。
...
- **基本仕様**: UI（`KeycodePanel` など）とプロトコルドライバーとの間でのキー設定のやり取りは、テキストの文字列処理ではなく、`UniversalAction` AST データ構造を経由して型安全に行う。`UniversalAction` は全ファームの特殊キーを同一視するためのものではなく、通常キー、透過・無操作、修飾、共通レイヤー操作などの共有可能な意味表現を担う。パレットへの掲載可否とRawキーは確定済みのファームウェアまたは接続プロトコルに従う。
